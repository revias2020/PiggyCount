...
const _tutorialSections = <(String, String)>[
  ...
  (
    '云服务、同步与数据',
    '「云服务」配置 WebDAV 或 S3；测通后可在「同步」把全部账本和分类/标签与网盘工作区对齐（先预览再写入；不是实时自动同步）。\n'
        '「数据管理」用于本机 CSV 导入导出。导入须带表头，先做列名映射；可打开「AI 智能映射」用文本模型预填列名、分类和标签。写入时屏幕中央显示进度，完成前不能操作。分类管理的导入同样有进度层。'
  ),
  (
    '桌面小组件',
