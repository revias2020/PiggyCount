...
const _tutorialSections = <(String, String)>[
  ...
  (
    ...
    '明细页右下角点「记一笔」打开底部弹层：选择支出/收入、金额、分类、标签、备注与时间，保存即可。'
        '新建时备注旁可语音记账，或用相机拍照/相册走识别入账；手动账单本身仍不支持附图。',
  ),
  (
    '智能记账',
    '长按「记一笔」展开扇形：拍照、语音、图片。\n'
        '· 拍照 / 图片：用视觉模型识别支付截图后确认入账。\n'
        '· 语音：须先在 AI 设置启用识别引擎（Vosk / Whisper 听写，或 AI 语音直接记账）；未启用时弹层内引导去设置。\n'
        '· 截图自动：在「我的」打开开关；Android 首次开启会扫描并固定监听目录（可改），仅目录内且含截图关键词的新图会入账；空目录时红字提示不可用；iOS 需按快捷指令引导配置；进度与结果走通知并自动入账。\n'
  ...
  (
    ...
    '「我的 → AI 设置」：\n'
        ...
        '· 能力绑定：分别为「文本对话」「图片理解」「语音直接记账」指定服务商（文本用于听写结构化与 CSV 智能映射）。\n'
        '· 语音识别：默认未启用；可选 Vosk / Whisper 离线包或 AI 语音模型。\n'
        '密钥只保存在本机。',
  ),
  (
    '分类与标签',
